...
function getUsuarios() {
  try { return JSON.parse(localStorage.getItem("bolao26_usuarios")||"{}"); } catch { return {}; }
}
function saveUsuarios(u) { localStorage.setItem("bolao26_usuarios", JSON.stringify(u)); }
function getUsuarioAtual() {
  try { return JSON.parse(sessionStorage.getItem("bolao26_user")||"null"); } catch { return null; }
}
function setUsuarioAtual(u) { sessionStorage.setItem("bolao26_user", JSON.stringify(u)); }
function logoutUsuario() { sessionStorage.removeItem("bolao26_user"); }
...
function LoginView({onLogin}) {
  ...
  const entrar = () => {
    const n = nome.trim(); if(!n||!senha){setErro("Preencha nome e senha.");return;}
    const usuarios = getUsuarios();
    if(modo==="cadastro") {
      if(usuarios[n.toLowerCase()]){setErro("Nome já cadastrado. Tente outro.");return;}
      if(senha.length<3){setErro("Senha muito curta (mínimo 3 caracteres).");return;}
      usuarios[n.toLowerCase()] = {nome:n, senha, palpites:[]};
      saveUsuarios(usuarios);
      setUsuarioAtual({nome:n});
      onLogin({nome:n});
    } else {
      const u = usuarios[n.toLowerCase()];
      if(!u){setErro("Nome não encontrado. Faça o cadastro.");return;}
      if(u.senha!==senha){setErro("Senha incorreta.");return;}
      setUsuarioAtual({nome:u.nome});
      onLogin({nome:u.nome});
    }
  };

  return(
    <div style={{minHeight:"100vh",background:"var(--dark)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"20px 14px"}}>
      <div style={{fontSize:"clamp(38px,10vw,52px)",marginBottom:8,filter:"drop-shadow(0 0 24px rgba(240,192,64,.8))"}}>🏆</div>
      <h1 style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:"clamp(38px,12vw,64px)",letterSpacing:".07em",background:"linear-gradient(180deg,#fff 0%,#f0c040 45%,#c8922a 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",lineHeight:.9,marginBottom:4,textAlign:"center"}}>COPA 2026</h1>
      <p style={{color:"var(--muted)",fontSize:12,marginBottom:24,fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:".15em"}}>BOLÃO OFICIAL</p>
      <div className="card" style={{padding:"24px 20px",width:"100%",maxWidth:380}}>
        {/* Tabs */}
        <div style={{display:"flex",marginBottom:20,background:"rgba(255,255,255,.05)",borderRadius:8,padding:3}}>
          {[["login","🔑 ENTRAR"],["cadastro","✏️ CADASTRAR"]].map(([m,l])=>(
            <button key={m} onClick={()=>{setModo(m);setErro("");}} style={{flex:1,padding:"8px",background:modo===m?"rgba(240,192,64,.2)":"transparent",border:`1px solid ${modo===m?"rgba(240,192,64,.4)":"transparent"}`,borderRadius:6,cursor:"pointer",color:modo===m?"#f0c040":"var(--muted)",fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:12,letterSpacing:".1em",transition:"all .2s"}}>
              {l}
            </button>
          ))}
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          <input className="field" placeholder="Seu nome..." value={nome} onChange={e=>setNome(e.target.value)} autoFocus/>
          <input className="field" type="password" placeholder="Senha..." value={senha} onChange={e=>setSenha(e.target.value)} onKeyDown={e=>e.key==="Enter"&&entrar()}/>
          {erro&&<p style={{color:"var(--red)",fontSize:12,fontFamily:"'Barlow',sans-serif"}}>{erro}</p>}
          <button className="btn-gold" style={{padding:"13px",fontSize:15,marginTop:4}} onClick={entrar}>
            {modo==="login"?"ENTRAR →":"CRIAR CONTA →"}
          </button>
        </div>
        {modo==="login"&&(
          <p style={{textAlign:"center",color:"var(--muted)",fontSize:11,marginTop:12,lineHeight:1.5}}>
            Não tem conta? Clique em <strong style={{color:"#f0c040"}}>CADASTRAR</strong> acima.
          </p>
        )}
        {modo==="cadastro"&&(
          <p style={{textAlign:"center",color:"var(--muted)",fontSize:11,marginTop:12,lineHeight:1.5}}>
            Cada conta pode fazer até <strong style={{color:"#f0c040"}}>3 palpites</strong> independentes.
          </p>
        )}
      </div>
    </div>
...
function MeusPalpitesView({usuario, palpites, resultados, setAba, setEditando, bloqueado}) {
  const meus = palpites.filter(p => p.autor===usuario.nome);
  const maxPts = Math.max(...palpites.map(p=>p.pts||0), 1);
    ...
    <div className="fadein section">
      <div className="card" style={{padding:"14px 16px",marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:18,color:"#f0c040",letterSpacing:".1em"}}>👤 MEUS PALPITES</div>
            <div style={{fontSize:11,color:"var(--muted)",marginTop:2}}>{meus.length}/3 palpites feitos</div>
          </div>
          {!bloqueado && meus.length < 3 && (
            <button className="btn-gold" style={{padding:"9px 16px",fontSize:13}} onClick={()=>setAba("novo")}>
              + NOVO
            </button>
          )}
        </div>
        {meus.length===0?(
          <div style={{textAlign:"center",padding:"20px 0"}}>
            ...
            <p style={{color:"var(--muted)",fontSize:13,marginBottom:12}}>Você ainda não fez nenhum palpite!</p>
            {!bloqueado&&<button className="btn-gold" style={{padding:"11px 22px",fontSize:14,display:"inline-block"}} onClick={()=>setAba("novo")}>FAZER MEU PALPITE</button>}
          </div>
        ):(
          meus.map((p,i)=>{
            const posGeral = [...palpites].sort((a,b)=>(b.pts||0)-(a.pts||0)).findIndex(x=>x.id===p.id);
            return(
              <div key={p.id} className="card-dark" style={{padding:"12px 14px",marginBottom:8,cursor:"pointer"}} onClick={()=>setEditando(p)}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:16,color:"#f0c040",letterSpacing:".08em"}}>PALPITE {i+1}</div>
                    <div style={{fontSize:11,color:"var(--muted)",marginTop:2}}>
                      🏆 {p.bracket?.campeao||"Sem campeão"} · {new Date(p.criadoEm||0).toLocaleDateString("pt-BR")}
                    </div>
                  </div>
                  <div style={{textAlign:"right"}}>
                    <div style={{fontFamily:"'Bebas Neue',Impact,sans-serif",fontSize:22,color:"#f0c040",lineHeight:1}}>{p.pts||0}</div>
                    <div style={{fontSize:9,color:"var(--muted)"}}>pts · {posGeral+1}º geral</div>
                  </div>
                </div>
                <div style={{marginTop:8,display:"flex",gap:6,flexWrap:"wrap"}}>
                  {p.bracket?.semi?.filter(Boolean).map(t=>(
                    <span key={t} style={{fontSize:10,background:"rgba(240,192,64,.1)",border:"1px solid rgba(240,192,64,.25)",borderRadius:6,padding:"2px 8px",color:"#f0c040",fontFamily:"'Barlow Condensed',sans-serif"}}>🏅 {t}</span>
                  ))}
                </div>
                <div style={{marginTop:6,display:"flex",gap:5,flexWrap:"wrap"}}>
                  {Object.entries(p.premios||{}).slice(0,3).map(([k,v])=>v&&(
                    <span key={k} style={{fontSize:10,background:"rgba(244,143,177,.08)",border:"1px solid rgba(244,143,177,.2)",borderRadius:6,padding:"2px 7px",color:"#f48fb1",fontFamily:"'Barlow Condensed',sans-serif"}}>{v}</span>
                  ))}
                </div>
                <div style={{marginTop:7,fontSize:10,color:"rgba(79,195,247,.7)",fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:".08em"}}>
...
export default function App() {
  ...
  const [palpiteEditando, setPalpiteEditando] = useState(null);
  const TOTAL = 15;
  ...
  const iniciar = () => {
    ...
    const meusPalpites = palpites.filter(p=>p.autor===n);
    if(meusPalpites.length>=3){alert("Você já fez 3 palpites! Limite máximo atingido.");return;}
    const numPalpite = meusPalpites.length + 1;
    setPalpite({id:Date.now(),nome:`${n} — Palpite ${numPalpite}`,autor:n,numPalpite,grupos:{},terceiros:[],premios:{},bracket:B0()});
  ...
  const iniciarComFavoritos = () => {
    const n = usuario.nome;
    const meusPalpites = palpites.filter(p=>p.autor===n);
    if(meusPalpites.length>=3){alert("Você já fez 3 palpites!");return;}
    const numPalpite = meusPalpites.length + 1;
    if(false){alert("");}
  ...
  const view = () => {
    ...
    if(aba==="meus") {
      if(palpiteEditando) return (
        <div className="fadein">
          <div style={{padding:"8px 14px",maxWidth:560,margin:"0 auto"}}>
            <button className="btn-ghost" style={{padding:"8px 14px",fontSize:12,marginBottom:8}} onClick={()=>setPalpiteEditando(null)}>← VOLTAR</button>
          </div>
          <Bracket palpite={palpiteEditando} onPick={()=>{}} onCampeao={()=>{}} onBack={()=>setPalpiteEditando(null)} onFinalizar={()=>setPalpiteEditando(null)} resultados={resultados} rdOnly/>
          <div style={{padding:"0 14px",maxWidth:560,margin:"0 auto"}}>
            <BotaoCompartilhar palpite={palpiteEditando} pos={[...palpitesComPts].sort((a,b)=>(b.pts||0)-(a.pts||0)).findIndex(x=>x.id===palpiteEditando.id)}/>
          </div>
        </div>
      );
      return <MeusPalpitesView usuario={usuario} palpites={palpitesComPts} resultados={resultados} setAba={setAba} setEditando={setPalpiteEditando} bloqueado={bloqueado}/>;
    };
    if(aba==="novo"){
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        <nav className="nav">
          {[{id:"home",l:"🏠"},{id:"meus",l:"👤 MEUS"},{id:"novo",l:"✏️ PALPITAR"},{id:"ranking",l:"🏆 RANKING"},{id:"grupos",l:"⚽ GRUPOS"},{id:"admin",l:"🔐",admin:true}].map(({id,l,admin}) => (
            <button key={id} className={`nav-tab ${aba===id?"active":""} ${admin?"atab":""}`} onClick={()=>{if(id!=="palpite")setAba(id);}}>{l}</button>
          ))}
        </nav>
        {/* Banner usuário logado */}
        <div style={{background:"rgba(0,0,0,.3)",borderBottom:"1px solid rgba(255,255,255,.06)",padding:"5px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:11,color:"var(--muted)",fontFamily:"'Barlow Condensed',sans-serif"}}>
            👤 <strong style={{color:"#f0c040"}}>{usuario.nome}</strong> · {palpites.filter(p=>p.autor===usuario.nome).length}/3 palpites
          </span>
          <button onClick={()=>{logoutUsuario();setUsuario(null);}} style={{background:"transparent",border:"none",color:"rgba(255,82,82,.5)",cursor:"pointer",fontSize:11,fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:".08em"}}>
            SAIR
          </button>
        </div>
        {aba==="palpite"&&palpite&&(
