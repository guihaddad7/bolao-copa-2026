...
function gerarImagemBracket(palpite, pos) {
  ...
  ctx.lineWidth = 2;
  ctx.strokeRect(8,8,584,804);

  // Título
  ctx.fillStyle = "#f0c040";
  ctx.font = "bold 48px 'Arial Black', Impact, sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("🏆 BOLÃO COPA 2026", 300, 65);

  // Nome do participante
  ctx.fillStyle = "#ffffff";
  ctx.font = "bold 28px Arial, sans-serif";
  ctx.fillText(palpite.nome, 300, 105);

  // Posição no ranking
  if (pos !== undefined) {
    ctx.fillStyle = pos===0?"#f0c040":pos===1?"#c0c0d0":pos===2?"#c87832":"#7a8aaa";
    ctx.font = "bold 20px Arial, sans-serif";
    ctx.fillText(pos===0?"🥇 1º lugar":pos===1?"🥈 2º lugar":pos===2?"🥉 3º lugar":`${pos+1}º lugar`, 300, 135);
  }
  ...
  ctx.textAlign = "center";
  ctx.fillText("⚽ MEU CAMPEÃO", 300, 185);
  ctx.font = "bold 32px Arial, sans-serif";
  ctx.fillStyle = "#ffffff";
  const camp = palpite.bracket?.campeao;
  ctx.fillText(camp ? `${fl(camp)} ${nm(camp)}` : "Não escolhido", 300, 225);

  // Divisor
  ctx.strokeStyle = "rgba(240,192,64,0.2)";
  ctx.beginPath(); ctx.moveTo(40,245); ctx.lineTo(560,245); ctx.stroke();

  // Finalistas
  ctx.fillStyle = "#f0c040";
  ctx.font = "bold 14px Arial, sans-serif";
  ctx.fillText("FINALISTAS", 300, 270);
  const semi = palpite.bracket?.semi?.filter(Boolean) || [];
  ctx.fillStyle = "#ccd";
  ctx.font = "18px Arial, sans-serif";
  ctx.fillText(semi.length > 0 ? semi.map(t=>`${fl(t)} ${nm(t)}`).join("  ×  ") : "—", 300, 298);

  // Divisor
  ctx.strokeStyle = "rgba(240,192,64,0.2)";
  ctx.beginPath(); ctx.moveTo(40,315); ctx.lineTo(560,315); ctx.stroke();

  // Prêmios individuais
  ctx.fillStyle = "#f48fb1";
  ctx.font = "bold 14px Arial, sans-serif";
  ctx.fillText("🏅 PRÊMIOS INDIVIDUAIS", 300, 342);
  ...
  premiosData.forEach(({label, id}, i) => {
    ...
    ctx.beginPath();
    ctx.roundRect(40, y-22, 520, 34, 8);
    ctx.fill();
    // Label
    ...
    ctx.font = "bold 14px Arial, sans-serif";
    ctx.textAlign = "right";
    ctx.fillText(palpite.premios?.[id] || "—", 545, y);
  });

  // Divisor
  ctx.strokeStyle = "rgba(240,192,64,0.2)";
  ctx.beginPath(); ctx.moveTo(40,590); ctx.lineTo(560,590); ctx.stroke();

  // Grupos highlight
  ctx.fillStyle = "#4fc3f7";
  ctx.font = "bold 14px Arial, sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("🌎 PALPITES DOS GRUPOS (selecionados)", 300, 618);

  const gruposDestaque = ["C","J","I","H"].map(g => ({
    g, p: palpite.grupos[g]
  })).filter(x => x.p?.primeiro);

  gruposDestaque.forEach(({g, p}, i) => {
    const x = 80 + i * 115;
    ctx.fillStyle = "rgba(79,195,247,0.1)";
    ctx.beginPath();
    ctx.roundRect(x-55, 630, 110, 70, 8);
    ctx.fill();
    ...
    ctx.textAlign = "center";
    ctx.fillText(`GRP ${g}`, x, 648);
    ctx.fillStyle = "#ffffff";
    ctx.font = "12px Arial, sans-serif";
    if (p.primeiro) ctx.fillText(`1º ${fl(p.primeiro)}`, x, 666);
    if (p.segundo) ctx.fillText(`2º ${fl(p.segundo)}`, x, 682);
    if (p.terceiro) ctx.fillText(`3º ${fl(p.terceiro)}`, x, 698);
  });
  ...
  ctx.font = "12px Arial, sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("bolao-copa-2026-five-lilac.vercel.app", 300, 785);
  ctx.fillStyle = "rgba(255,255,255,0.2)";
  ctx.fillText("11 JUN – 19 JUL 2026 · EUA · MÉXICO · CANADÁ", 300, 805);
...
// ─── NOVO ──────────────────────────────────────────────────────────
function NovoView({nome, setNome, iniciar, partics, bloqueado}) {
  if (bloqueado) return (
    <div className="fadein section">
...
export default function App() {
  ...
  const view = () => {
    ...
    if(aba==="novo"){
      ...
      return <NovoView nome={nome} setNome={setNome} iniciar={iniciar} partics={palpites} bloqueado={bloqueado}/>;
    }
